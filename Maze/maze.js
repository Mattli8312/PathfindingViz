...
async function RenderMaze(maze_object){
    ...
    for(var i = 1; i < maze_object.height * 2 + 1; i+=2){
        for(var j = 1; j < maze_object.width * 2 + 1; j+=2){
            ...
            if(i == 1){
                wall = document.getElementById((i-1) + ',' + (j)); wall.style.background = "white";
                wall = document.getElementById((i-1) + ',' + (j+1)); wall.style.background = "white";
            }
            if(j == 1){
                wall = document.getElementById((i) + ',' + (j-1)); wall.style.background = "white";
                wall = document.getElementById((i+1) + ',' + (j-1)); wall.style.background = "white";
            }
